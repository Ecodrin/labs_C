#include "../include/client_shared_memory.hpp"

ClientSharedMemory::ClientSharedMemory(const char *ftok_shm_file, int ftok_shm_id, const char *ftok_sem_file,
                                       int ftok_sem_id,
...
void ClientSharedMemory::set_sem_val(unsigned short *op) const {
...
void ClientSharedMemory::get_sem_val(unsigned short *res) const {
...
void ClientSharedMemory::change_sem_val(short *op) const {
...
void ClientSharedMemory::change_sem_val(short op, unsigned short num_sem) const {
...
void ClientSharedMemory::send(const void *msg, size_t size_msg) {
    if(msg == nullptr) {
        throw std::invalid_argument("msg = null ptr in ClientSharedMemory::send");
    }
    short op[2];
    op[0] = 0;
    op[1] = -1;
...
void ClientSharedMemory::rcv(void *msg, size_t size_msg) {
    if(msg == nullptr) {
        throw std::invalid_argument("msg = null ptr in ClientSharedMemory::rcv");
    }
